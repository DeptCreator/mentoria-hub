...
-- All inserts are idempotent and can be safely re-run

-- Categories
INSERT INTO categories (name, slug, description, color)
SELECT name, slug, description, color FROM (VALUES
...
-- Opportunities
INSERT INTO opportunities (title, description, category, format, grade_min, grade_max, deadline, requirements, benefits, link, is_featured)
SELECT title, description, category, format, grade_min, grade_max, deadline, requirements, benefits, link, is_featured FROM (VALUES
...
) AS v(title, description, category, format, grade_min, grade_max, deadline, requirements, benefits, link, is_featured)
WHERE NOT EXISTS (SELECT 1 FROM opportunities WHERE title = v.title);

-- Courses
INSERT INTO courses (title, description, category, level, duration_hours, is_published)
SELECT title, description, category, level, duration_hours, is_published FROM (VALUES
...
) AS v(title, description, category, level, duration_hours, is_published)
WHERE NOT EXISTS (SELECT 1 FROM courses WHERE title = v.title);

-- Lessons for Mathematics Fundamentals
INSERT INTO lessons (course_id, title, description, video_url, video_type, order_index, duration_minutes, quiz_questions, materials)
SELECT course_id, title, description, video_url, video_type, order_index, duration_minutes, quiz_questions::jsonb, materials FROM (VALUES
...
) AS v(course_id, title, description, video_url, video_type, order_index, duration_minutes, quiz_questions, materials)
WHERE v.course_id IS NOT NULL
AND NOT EXISTS (SELECT 1 FROM lessons WHERE course_id = v.course_id AND title = v.title);
...
SELECT name, description, icon, condition_type, condition_value FROM (VALUES
...
WHERE NOT EXISTS (SELECT 1 FROM achievements WHERE name = v.name);
